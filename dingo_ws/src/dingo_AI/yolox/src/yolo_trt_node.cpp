...
public:
    YoloTrtNode() : privateNh_("~") {
        ...
        sub_ = nh_.subscribe(imageTopic_, 1, &YoloTrtNode::imageCallback, this, ros::TransportHints().tcpNoDelay());
        ROS_INFO_STREAM("YOLO TensorRT C++ tracker subscribed to " << imageTopic_ << " model=" << modelPath_);
    }
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        ROS_FATAL("failed to start yolo_trt_node: %s", exc.what());
        return 1;
    }
    return 0;
}
